...
public class RandBot extends GamePlayer {
	...
	public CommandOption makeAChoice(CommandOption[] options) {
		if (this.difficulty == Difficulty.EASY)
			// return easyTestingMode(options); //TODO Test for changing ai
			// difficulty only by zxn
				...
				if (opt.isBuildable()) {
					if (buildCnt == num) {
						...
						if (!opt.isAvailableFree() && opt.isOptions()) {
							this.makeABuyDecision(
									SimpleResList.buildCostList(opt.getCard()),
									opt.leftSRL, opt.rightSRL);
						}
			...
			for (CommandOption opt : options)
				if (opt.isBuildable()) {
					if (opt.getCard().getColor() == CardColor.YELLOW)
						yellowCards.add(opt);
					else if ((opt.getCard().getColor() == CardColor.BROWN || opt
							.getCard().getColor() == CardColor.GREY)
							&& opt.getNeedToBuild() == 0)
						resCards.add(opt);
				...
				if (opt.isBuildable()
					...
					if (opt.getCard().getColor() == CardColor.BROWN
						...
						if (needed)
							...
							|| opt.getCard().getColor() == CardColor.GREY) {
						SimpleResList tmp = opt.getCard().getResList();
						SimpleResList boardList = board.getResList();
						boolean needed = true;
						for (int i = 1; i < 8; i++)
	...
	private CommandOption hardMode(CommandOption[] options) {
		...
		if (MainBackGround.age == 1) {
			...
			if (board.getColorCount(CardColor.YELLOW) == 0
					&& yellowCards.size() > 0) {
				int num = 0;
				for (int i = 0; i < yellowCards.size(); i++)
					if (yellowCards.get(i).getCard().getName()
							.equals("Marketplace"))
						num = i;
				opt = yellowCards.get(num);
				opt.setCommand(Command.BUILD_CARD);
			...
			int left = (board.leftNeighbor.goods.containsKey(Resource.SHEILD)) ? board.leftNeighbor.goods
					.get(Resource.SHEILD) : 0;
			int right = (board.rightNeighbor.goods.containsKey(Resource.SHEILD)) ? board.rightNeighbor.goods
					.get(Resource.SHEILD) : 0;
			int self = (board.goods.containsKey(Resource.SHEILD)) ? board.goods
					.get(Resource.SHEILD) : 0;

			// No shield, Red is free
			if (left + right != 0 && freeRedCards.size() > 0) {
				int num = 0;
				// The card that can build by the left neighbor have a higher
				// order
				for (int i = 0; i < freeRedCards.size(); i++) {
					SimpleResList tmp = freeRedCards.get(i).getCard()
							.getResList();
					int j = 1;
					while (j < 8 && tmp.srl[j] == 0)
						j++;
			...
			if (brownCards.size() > 0 || greyCards.size() > 0
					|| orCards.size() > 0) {
				...
				for (int i = 0; i < orCards.size(); i++) {
					...
					else if (board.getResList().srl[j]
							+ board.getResList().srl[k] == 1)
						lowerOrderList.add(orCards.get(i));
					else
						lastOrderList.add(orCards.get(i));
				}
				for (int i = 0; i < brownCards.size(); i++) {
					SimpleResList tmp = brownCards.get(i).getCard()
							.getResList();
			...
			if ((self <= left || self <= right) && redCards.size() > 0) {
				...
				for (int i = 1; i < redCards.size(); i++)
					if (redCards.get(i).getNeedToBuild() < redCards.get(num)
							.getNeedToBuild())
						num = i;
				opt = redCards.get(num);
				opt.setCommand(Command.BUILD_CARD);
			...
			if (freeYellowCards.size() > 0) {
				...
				for (int i = 0; i < freeYellowCards.size(); i++)
					if (freeYellowCards.get(i).getCard().getName()
							.equals("Forum")
							|| freeYellowCards.get(i).getCard().getName()
									.equals("Caravansery"))
				...
				if (board.goods.get(Resource.COIN) < 3 && coin.size() > 0)
					...
					opt = freeYellowCards.get(rand.nextInt(freeYellowCards
							.size()));
				opt.setCommand(Command.BUILD_CARD);
				if (!opt.isAvailableFree() && opt.isOptions()) {
					this.makeABuyDecision(
			...
			if (blueCards.size() > 0) {
				...
				for (int i = 1; i < blueCards.size(); i++)
					if (blueCards.get(i).getCard().goods.get(Resource.VP) > blueCards
							.get(num).getCard().goods.get(Resource.VP))
						num = i;
				opt = blueCards.get(num);
				opt.setCommand(Command.BUILD_CARD);
				if (!opt.isAvailableFree() && opt.isOptions()) {
			...
			for (int i = 0; i < options.length; i++)
				if (options[i].getCard().getColor() == CardColor.RED) {
					opt = options[i];
					opt.setCommand(Command.SELL_CARD);
					System.out.println("Hard MakeChoice: " + opt);
					return opt;
			...
			// Sell Blue
			for (int i = 0; i < options.length; i++)
				if (options[i].getCard().getColor() == CardColor.BLUE) {
					opt = options[i];
					opt.setCommand(Command.SELL_CARD);
					System.out.println("Hard MakeChoice: " + opt);
					return opt;
			...
			// Sell Green
			for (int i = 0; i < options.length; i++)
				if (options[i].getCard().getColor() == CardColor.GREEN) {
					opt = options[i];
					opt.setCommand(Command.SELL_CARD);
					System.out.println("Hard MakeChoice: " + opt);
					return opt;
			...
			for (CommandOption opt : options)
				if (opt.isBuildable()) {
					if (opt.getCard().getColor() == CardColor.RED) {
						...
						blueCards.add(opt);
					else if (opt.getCard().getColor() == CardColor.PURPLE
							&& !opt.getCard().getName()
									.equals("Scientists Guild"))
						purpleCards.add(opt);
				}
			...
			// Sell Purple
			for (int i = 0; i < options.length; i++)
				if (options[i].getCard().getColor() == CardColor.PURPLE) {
					opt = options[i];
					opt.setCommand(Command.SELL_CARD);
					System.out.println("Hard MakeChoice: " + opt);
					return opt;
	...
	/**
	 * TODO Changed by zxn 4-2, 4-5, bug-2 Calculate resources needed to buy
	 * from neighbors TODO Need to improve
	 */
