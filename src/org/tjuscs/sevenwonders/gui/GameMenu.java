...
public class GameMenu extends Group {
	...
	public GameMenu() {
		...
		soundButton.setGraphic(new ImageView(ResManager
				.getImage("img_sound.png")));
		...
		returnButton.setGraphic(new ImageView(ResManager
				.getImage("img_return.png")));
		...
		exitButton
				.setGraphic(new ImageView(ResManager.getImage("img_exit.png")));
		...
		down.getKeyFrames().addAll(
				new KeyFrame(Duration.ZERO, new KeyValue(
						...
						new KeyValue(botButton.layoutYProperty(), -50.0),
						new KeyValue(soundButton.layoutYProperty(), -50.0),
						new KeyValue(returnButton.layoutYProperty(), -50.0),
						new KeyValue(exitButton.layoutYProperty(), -50.0)),
				new KeyFrame(new Duration(100), new KeyValue(menuBackground
						.layoutYProperty(), -265.0), new KeyValue(botButton
						.layoutYProperty(), 15.0), new KeyValue(soundButton
						.layoutYProperty(), 15.0), new KeyValue(returnButton
						.layoutYProperty(), 15.0), new KeyValue(exitButton
						.layoutYProperty(), 15.0)));

		final Timeline up = new Timeline();
		up.getKeyFrames().addAll(
				new KeyFrame(Duration.ZERO, new KeyValue(
						menuBackground.layoutYProperty(), -265.0),
						new KeyValue(botButton.layoutYProperty(), 15.0),
						new KeyValue(soundButton.layoutYProperty(), 15.0),
						new KeyValue(returnButton.layoutYProperty(), 15.0),
						new KeyValue(exitButton.layoutYProperty(), 15.0)),
				new KeyFrame(new Duration(100), new KeyValue(menuBackground
						.layoutYProperty(), -335.0), new KeyValue(botButton
						.layoutYProperty(), -50.0), new KeyValue(soundButton
						.layoutYProperty(), -50.0), new KeyValue(returnButton
						.layoutYProperty(), -50.0), new KeyValue(exitButton
						.layoutYProperty(), -50.0)));

		// TODO Added by zxn 4-7 new-2
		botButton.setOnMouseClicked(new EventHandler<MouseEvent>() { // Bot
					// Difficulty

					public void handle(MouseEvent event) {
						int botNum = Manager.getKernel().getNumPlayers() - 1;
						// Create bot difficulty dialog
						InnerDialog botDiffDialog = new InnerDialog(400,
								50 * botNum + 80);
						...
						btn.setOnMouseClicked(new EventHandler<MouseEvent>() {
							...
							public void handle(MouseEvent event) {
								...
								for (int i = 0; i < bn; i++) {
									...
									Manager.getKernel().setAIDifficulty(i + 1,
											diffs[i]);
								...
								if (event.getCode().equals(KeyCode.ENTER)) {
									...
									for (int i = 0; i < bn; i++) {
										...
										if (botDiffs.get(i).getSelectionModel()
												...
												.equals("Easy"))
											diffs[i] = Difficulty.EASY;
										Manager.getKernel().setAIDifficulty(
												i + 1, diffs[i]);
		...
		// gm = this;
		// ImageView menuBackground = new
		// ImageView(ResManager.getImage("bg.png"));
		...
		//
		// final Button soundButton = new Button();
		// soundButton.setGraphic(new
		// ImageView(ResManager.getImage("img_sound.png")));
		// soundButton.setLayoutX(150.0);
		// // optionButton.setLayoutY(20.0);
		// soundButton.setLayoutY(-50.0);
		// setButtonEffect(soundButton);
		...
		//
		// Button returnButton = new Button();
		// returnButton.setGraphic(new
		// ImageView(ResManager.getImage("img_return.png")));
		// returnButton.setLayoutX(220.0);
		// // returnButton.setLayoutY(20.0);
		// returnButton.setLayoutY(-50.0);
		// setButtonEffect(returnButton);
		...
		//
		// Button exitButton = new Button();
		// exitButton.setGraphic(new
		// ImageView(ResManager.getImage("img_exit.png")));
		// exitButton.setLayoutX(290.0);
		// // exitButton.setLayoutY(20.0);
		// exitButton.setLayoutY(-50.0);
		// setButtonEffect(exitButton);
		...
		// KeyValue(menuBackground.layoutYProperty(), -335.0), new KeyValue(
		// soundButton.layoutYProperty(), -50.0), new
		// KeyValue(returnButton.layoutYProperty(), -50.0),
		// new KeyValue(exitButton.layoutYProperty(), -50.0)),
		// new KeyFrame(new Duration(100), new
		// KeyValue(menuBackground.layoutYProperty(), -265.0), new KeyValue(
		// soundButton.layoutYProperty(), 15.0), new
		// KeyValue(returnButton.layoutYProperty(), 15.0),
		// new KeyValue(exitButton.layoutYProperty(), 15.0)));
		//
		// final Timeline up = new Timeline();
		// up.getKeyFrames().addAll(
